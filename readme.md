...
The goal was to transform unstructured clinical narrative text into strictly formatted, FHIR like JSON. For this initial project I chose a custom schema the LLM had never seen before, a fictitious schema for my “Solaris EMR”. Given the hardware available this project used **QLoRA (Quantized Low-Rank Adaptation)** to put complex proprietary schemas and logic rules directly into the model weights, achieving higher reliability and lower latency than Zero Shot approaches. I had tried using Zero Shot and prompt engineering to get the LLM in my RAG Retrieval project to output specific schemas from information in my PDF files that had been ingested, and whilst it did work quite successfully had several issues:
...
**Purpose:** Generates synthetic clinical training data to ensure schema adherence.

* **Function:** Manufactures 500 clinical notes paired with strictly formatted JSON.  
* **Key Logic:** Implements the SolarisEMRClinicalSummary resource, negative Constraints (enforcing null values for missing data) to prevent hallucination and gives examples of repeating segments.  
* **Output:** Produces train.jsonl and val.jsonl.

### Training the model

**train_lora.py**

**Purpose:** Executes the QLoRA fine-tuning process.

* **Function:** Loads the Qwen 2.5 3B model in 4-bit quantization and trains Low-Rank Adapters (LoRA) on the synthetic dataset.  
* **Key Logic:**  
  * **Hardware Optimization:** Sets "Eager" attention, and fp16/bf16 values ensuring stability on Pascal-architecture GPUs (GTX 10-series).  
  * **Memory Management:** Uses Gradient Accumulation to simulate larger batch sizes within 8GB VRAM limits.  
* **Output:** Saves the adapter weights to ./Qwen2.5-3B-Clinical-Extract.

### Evaluation 

**evaluate_and_plot.py**

**Purpose:** Verifies training success using mathematical metrics and logic tests.

* **Function:**  
  1. Parses training logs to generate a **Loss Curve** (loss_curve.png) to visualise convergence.  
  2. Runs a "Test Suite" (Standard, Enhanced, Wildcard, and Edge Cases) against both the Base Model and the Fine-Tuned Model.  
  3. Performs JSON inspection to verify logic (e.g., did the model correctly split "500mg" into Value/Unit?).  
* **Output:** Generates a comparison report: evaluation_results.md.
...
## Generating Data (generate_dataset.py)
...
To fit the **Qwen 2.5 3B** model into my **GTX 1070 Ti (8GB VRAM)**, I utilised 4-bit quantization (NF4) and trained **LoRA Adapters:**
...
| **Gradient Accumulation** | 4 | Since Batch Size was 2, I used accumulation to simulate a Batch Size of 8. This stabilizes the gradient updates, preventing the training from being too 'noisy' |

Training on my **GTX 1070 Ti** presented some unique challenges due to its lack of **BFloat16** support and Tensor Cores. During training I was always stopped by errors from AMP: *RuntimeError: "_amp_foreach_non_finite_check_and_unscale_cuda" not implemented for 'BFloat16'*. To get around this lack of support for BF16 on my old GPU, there were two adjustments I had to make:

* **Force FP16:** I explicitly disabled AMP (Automatic Mixed Precision) and bf16 modes, forcing the model to run in pure float16.  
* **Eager Attention:** I disabled Flash Attention 2 (which requires newer architecture) and forced attn_implementation='eager' to ensure stability.
...
* *Pt John Doe, 55y, history of T2DM. Meds: Metformin 500mg bid.*  
